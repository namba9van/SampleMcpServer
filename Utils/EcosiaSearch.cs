...
/// HTML-страницы результатов. У Ecosia нет документированного JSON API без ключа, в отличие от
/// Marginalia и Baidu, поэтому используется извлечение данных из HTML, как у Mojeek и Yandex, с
/// теми же ограничениями: разметка страницы может измениться, поэтому используется несколько
/// резервных селекторов для повышения устойчивости. Если диагностика показывает отсутствие
/// найденных узлов результатов — значит, селекторы нужно обновить под изменившуюся разметку;
/// если диагностика показывает "challenge" — это защита от ботов, а не проблема селекторов.
/// </summary>
public static class EcosiaSearch
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // Разметка карточек результатов Ecosia исторически строилась вокруг блока
        // "mainline"/"result" с якорем result-title. Ниже перечислено несколько резервных
        // вариантов селекторов на случай, если разметка или имена классов изменились.
